...
def main():
    ...
    image_size = np.array([1226, 370])

    plt.figure()
    #ax = pt.plot_transform()
    ax = plt.axes(projection='3d')
    eps = 1e-16
    ax.axes.set_xlim3d(left=0.-eps, right=40+eps)
    ax.axes.set_ylim3d(bottom=0.-eps, top=40+eps) 
    ax.axes.set_zlim3d(bottom=0.-eps, top=40+eps) 
    ...
    colors = cycle("rgb")
    print(key_frames_indices,camera_pose_poses[i])
